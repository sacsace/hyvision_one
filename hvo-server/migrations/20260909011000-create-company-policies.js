...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    const [policyTable] = await sequelize.query(`
      SELECT EXISTS (
    ...
    const [revTable] = await sequelize.query(`
      SELECT EXISTS (
        SELECT FROM information_schema.tables
        WHERE table_schema = 'public' AND table_name = 'company_policy_revisions'
      ) AS exists;
    `);
    if (!revTable[0]?.exists) {
