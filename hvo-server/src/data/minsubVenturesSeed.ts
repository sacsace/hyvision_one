...
export const MINSUB_COMPANY = {
  name: 'Hyvision India',
  business_number: 'HVO-IN-2026-001',
  ceo_name: 'Hyvision',
  address: 'India',
  phone: '+91-0000-000000',
  email: 'admin@hyvision.in',
  website: 'https://www.hyvision.in',
  industry: 'Manufacturing / Vision Systems',
  employee_count: 25,
...
async function ensureMinsubCompany(tenantId: number) {
  ...
  if (!company) {
    ...
    if (existingFirst) {
      ...
      console.log('  ✅ 회사 id=1 → Hyvision India 로 갱신');
    } else {
      company = await (Company as any).create({
        tenant_id: tenantId,
        ...MINSUB_COMPANY,
      });
      console.log('  ✅ Hyvision India 회사 생성');
    }
  } else {
    await company.update(MINSUB_COMPANY);
    console.log(`  ✅ Hyvision India 회사 갱신 (id=${company.id})`);
  }
...
async function ensureUsers(tenantId: number, companyId: number) {
  ...
  const users = [
    { userid: 'root', username: 'Root User', email: 'root@hvo3.com', role: 'root' as const },
    { userid: 'admin', username: 'Admin User', email: 'admin@hvo3.com', role: 'admin' as const },
    { userid: 'user1', username: 'User One', email: 'user1@hvo3.com', role: 'user' as const },
  ];
