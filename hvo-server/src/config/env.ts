...
const loadEnvFiles = () => {
  ...
  // hvo-server 패키지 로컬 설정만 사용 (모노레포 루트 .env가 다른 제품 값을 덮지 않게)
...
/** MVS 등 타 제품 DB로 잘못 붙는 것을 차단 */
const assertHyvisionDbIsolation = () => {
  if (process.env.RAILWAY_ENVIRONMENT || process.env.NODE_ENV === 'test') {
    return;
  }

  const blockedDbNames = new Set(['mvs', 'mvs_test', 'mvs_prod', 'hvo']);
  const blockedUsers = new Set(['mvs_user']);
  ...
  const urlDbMatch = databaseUrl.match(/\/([a-z0-9_]+)(?:\?|$)/);
  const urlDbName = urlDbMatch?.[1] || '';

  if (blockedDbNames.has(dbName) || blockedDbNames.has(urlDbName)) {
    console.error('❌ DB isolation: Hyvision One cannot use DB_NAME/DATABASE_URL pointing at MVS legacy databases (mvs/hvo).');
    console.error('   Use DB_NAME=hyvision_one (PostgreSQL port 5433) only.');
    process.exit(1);
  }
  if (blockedUsers.has(dbUser)) {
    console.error('❌ DB isolation: DB_USER=mvs_user is not allowed for Hyvision One. Use hvo_user.');
    process.exit(1);
  }
  if (databaseUrl && (databaseUrl.includes('/mvs') || databaseUrl.includes('mvs_user'))) {
    console.error('❌ DB isolation: DATABASE_URL looks like an MVS connection. Refusing to start.');
    process.exit(1);
  }
};

assertHyvisionDbIsolation();
...
export const env = {
  ...
  /** HeresNow ↔ Hyvision One 근태 연동 (반드시 https://www.heresnow.in — apex는 API 없음) */
  HERESNOW_API_BASE_URL: process.env.HERESNOW_API_BASE_URL || 'https://www.heresnow.in',
  HVO_INTEGRATION_API_KEY: process.env.HVO_INTEGRATION_API_KEY || process.env.HERESNOW_INTEGRATION_API_KEY || '',
  INTEGRATION_DISPATCH_SECRET: process.env.INTEGRATION_DISPATCH_SECRET || '',
  HVO_WEBHOOK_BEARER: process.env.HVO_WEBHOOK_BEARER || '',
...
export const validateEnv = () => {
  ...
  if (env.NODE_ENV === 'production') {
    ...
    if (env.CORS_ORIGIN === 'http://localhost:3030') {
      errors.push('프로덕션 환경에서는 CORS_ORIGIN을 localhost로 설정하면 안됩니다.');
    }
  }

  // 로컬: Hyvision One 전용 DB명 강제 (MVS 혼선 방지)
  if (!process.env.RAILWAY_ENVIRONMENT && process.env.NODE_ENV !== 'test') {
    const dbName = String(env.DB_NAME || '').trim().toLowerCase();
    if (dbName && dbName !== 'hyvision_one' && !dbName.startsWith('hyvision_one')) {
      errors.push(`로컬 DB_NAME은 hyvision_one 이어야 합니다 (현재: ${env.DB_NAME}).`);
    }
  }
...
export const printEnvInfo = () => {
  console.log('🔧 환경 설정 정보 (Hyvision One):');
  console.log(`  - 환경: ${env.NODE_ENV}`);
  console.log(`  - 포트: ${env.PORT}`);
  console.log(`  - DB: ${env.DB_NAME}@${env.DB_HOST}:${env.DB_PORT} (user=${env.DB_USER})`);
