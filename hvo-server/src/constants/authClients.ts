 ...
 */
export const HVO_NOTIFIER_CLIENT = 'hvo_notifier';

export function isHvoNotifierClient(client?: string | null): boolean {
  return String(client || '').trim().toLowerCase() === HVO_NOTIFIER_CLIENT;
}
...
/** body.client / X-HVO-Client / User-Agent 로 알람 앱 로그인·요청 판별 */
export function resolveIsNotifierAuth(req: RequestLike): boolean {
  if (isHvoNotifierClient(typeof req.body?.client === 'string' ? req.body.client : null)) {
    return true;
  }
  if (isHvoNotifierClient(headerValue(req, 'x-hvo-client'))) {
    return true;
  }
  const ua = headerValue(req, 'user-agent');
  return /HVO-Notifier/i.test(ua);
}
