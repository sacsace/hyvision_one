...
export const reconcileTallyImportBatch = async ({
  ...
  const hvoTotals = emptyAmounts();
  const sourceLedgers = new Map<string, Amounts>();
  const hvoLedgers = new Map<string, Amounts>();
  ...
  for (const document of sourceDocuments) {
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
      ...
      const key = sourceLedgerKey(mapping?.hvoAccountId, line.ledgerName);
      if (!key) continue;
      const ledger = sourceLedgers.get(key) || emptyAmounts();
      addAmount(ledger, line.debit, line.credit);
      sourceLedgers.set(key, ledger);
      ledgerLabels.set(key, mapping?.hvoAccountName || String(line.ledgerName || key));
    }
  }

  for (const line of voucherLines) {
    addAmount(hvoTotals, line.debit, line.credit);
    const key = sourceLedgerKey(line.account_id, line.account_name);
    if (!key) continue;
    const ledger = hvoLedgers.get(key) || emptyAmounts();
    addAmount(ledger, line.debit, line.credit);
    hvoLedgers.set(key, ledger);
    if (!ledgerLabels.has(key)) ledgerLabels.set(key, String(line.account_name || key));
  }

  const allLedgerKeys = new Set([...sourceLedgers.keys(), ...hvoLedgers.keys()]);
  const ledgerDifferences = [...allLedgerKeys]
    .map((ledger) => {
      const source = sourceLedgers.get(ledger) || emptyAmounts();
      const hvo = hvoLedgers.get(ledger) || emptyAmounts();
      const difference = {
        debit: source.debit.minus(hvo.debit),
        credit: source.credit.minus(hvo.credit),
      };
      return { ledger, label: ledgerLabels.get(ledger) || ledger, source, hvo, difference };
    })
    .filter((row) => !row.difference.debit.isZero() || !row.difference.credit.isZero())
    .sort((a, b) => a.ledger.localeCompare(b.ledger));

  const debitDifference = sourceTotals.debit.minus(hvoTotals.debit);
  const creditDifference = sourceTotals.credit.minus(hvoTotals.credit);
