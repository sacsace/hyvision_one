...
const mapHvoEventToRecord = (event: HeresnowHvoEvent): HeresnowAttendanceRecord => {
...
const extractAttendanceRowsFromPayload = (payload: any): HeresnowAttendanceRecord[] => {
  ...
  if (Array.isArray(payload.events)) return payload.events.map((event: HeresnowHvoEvent) => mapHvoEventToRecord(event));
  return [];
};
...
export async function pullHeresnowAttendance(company: Company, options?: { since?: string; dryRun?: boolean }) {
  ...
  const apiKey =
    ...
    || process.env.HVO_INTEGRATION_API_KEY
  ...
  const url = new URL(`${baseUrl}/api/integrations/hvo/attendance`);
  ...
  // 여기서는 원격에 전달하지 않고 Hyvision One에서 월 범위를 강제 필터링한다.

  const headers: Record<string, string> = {
    Authorization: `Bearer ${apiKey}`,
    Accept: 'application/json',
    'x-hvo-api-key': apiKey
  };
  const webhookBearer = process.env.HVO_WEBHOOK_BEARER;
  if (webhookBearer) {
    headers['X-HVO-Webhook-Bearer'] = webhookBearer;
  }
...
export async function getHeresnowIntegrationStatus(company: Company) {
  ...
  const dispatchConfigured = Boolean(process.env.INTEGRATION_DISPATCH_SECRET || process.env.HVO_WEBHOOK_BEARER);
  const apiKeyHint =
  ...
  return {
    ...
    apiConfigured,
    dispatchConfigured,
    webhookPath: '/api/integrations/hvo/dispatch',
