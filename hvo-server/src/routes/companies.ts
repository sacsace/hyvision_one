...
// 모든 회사 조회 (테넌트별) — 단일회사 모드에서는 로그인 회사만
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    } else if (userRole !== 'root' && userRole !== 'audit') {
    ...
    const cacheKey = buildReferenceCacheKey([
      ...
      tenantId,
      userRole,
      isSingleCompanyMode() ? `c${userCompanyId}` : 'all',
    ]);
...
router.post(
  ...
  try {
    if (isSingleCompanyMode()) {
      return res.status(403).json({
        success: false,
        message: '단일 회사 모드에서는 회사를 추가할 수 없습니다.',
      });
    }
            const tenantId = (req as any).user.tenant_id;
    ...
    // hvo_start_date, hvo_end_date를 login_period_start, login_period_end로 매핑
    if (companyData.hvo_start_date) {
      companyData.login_period_start = companyData.hvo_start_date;
      delete companyData.hvo_start_date;
    }
    if (companyData.hvo_end_date) {
      companyData.login_period_end = companyData.hvo_end_date;
      delete companyData.hvo_end_date;
    }
...
router.put(
  ...
  try {
    ...
    // hvo_start_date, hvo_end_date를 login_period_start, login_period_end로 매핑
    if (updateData.hvo_start_date !== undefined) {
      updateData.login_period_start = updateData.hvo_start_date;
      delete updateData.hvo_start_date;
    }
    if (updateData.hvo_end_date !== undefined) {
      updateData.login_period_end = updateData.hvo_end_date;
      delete updateData.hvo_end_date;
    }

    // 단일 회사 모드: 사용 기간을 두지 않음
    if (isSingleCompanyMode()) {
...
router.delete('/:id', authenticateToken, requireRole(['root']), async (req, res) => {
  try {
    if (isSingleCompanyMode()) {
      return res.status(403).json({
        success: false,
        message: '단일 회사 모드에서는 회사를 삭제할 수 없습니다.',
      });
    }
