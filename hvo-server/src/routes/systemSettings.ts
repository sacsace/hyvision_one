...
router.post('/test-mail', authenticateToken, requireRole(['root', 'admin']), async (req, res) => {
  try {
    ...
    const companyName = String(companyRow.name || 'Hyvision One').trim();
    ...
    const html = `
      <div style="font-family:Segoe UI,Malgun Gothic,sans-serif;font-size:14px;color:#111827;line-height:1.55;max-width:640px;">
        ...
        <p style="margin:0 0 8px;">이 메일은 <strong>${companyName}</strong> Hyvision One 시스템의 SMTP 설정 테스트로 발송되었습니다.</p>
        ...
        <p style="margin:0 0 8px;">This message was sent as an SMTP configuration test from the <strong>${companyName}</strong> Hyvision One system.</p>
