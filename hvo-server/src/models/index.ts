...
import MfgBranch from './MfgBranch';
import MfgBinLocation from './MfgBinLocation';
import MfgDocumentSequence from './MfgDocumentSequence';
import MfgStockLedgerEntry from './MfgStockLedgerEntry';
import MfgPurchaseRequisition from './MfgPurchaseRequisition';
import MfgPurchaseRequisitionItem from './MfgPurchaseRequisitionItem';
import PurchaseApprovalSetting from './PurchaseApprovalSetting';
import MfgPurchaseOrder from './MfgPurchaseOrder';
import MfgPurchaseOrderItem from './MfgPurchaseOrderItem';
import MfgGoodsReceipt from './MfgGoodsReceipt';
import MfgGoodsReceiptItem from './MfgGoodsReceiptItem';
import MfgQualityInspection from './MfgQualityInspection';
import MfgDocumentAuditLog from './MfgDocumentAuditLog';
import MfgSalesOrder from './MfgSalesOrder';
import MfgSalesOrderItem from './MfgSalesOrderItem';
import MfgStockReservation from './MfgStockReservation';
import MfgDeliveryChallan from './MfgDeliveryChallan';
import MfgDeliveryChallanItem from './MfgDeliveryChallanItem';
import MfgCreditNote from './MfgCreditNote';
import MfgWorkCenter from './MfgWorkCenter';
import MfgBom from './MfgBom';
import MfgBomVersion from './MfgBomVersion';
import MfgBomItem from './MfgBomItem';
import MfgRouting from './MfgRouting';
import MfgRoutingOperation from './MfgRoutingOperation';
import MfgProductionPlan from './MfgProductionPlan';
import MfgMaterialRequirement from './MfgMaterialRequirement';
import MfgWorkOrder from './MfgWorkOrder';
import MfgMaterialIssue from './MfgMaterialIssue';
import MfgMaterialIssueItem from './MfgMaterialIssueItem';
import MfgProductionEntry from './MfgProductionEntry';
import MfgScrapEntry from './MfgScrapEntry';
import MfgStandardCost from './MfgStandardCost';
import MfgTaxConfiguration from './MfgTaxConfiguration';
import MfgPeriodLock from './MfgPeriodLock';
import MfgAutoJournalLink from './MfgAutoJournalLink';
import MfgBudgetLine from './MfgBudgetLine';
import MfgHqReportSnapshot from './MfgHqReportSnapshot';
import MfgKpiDaily from './MfgKpiDaily';
import HqFxRate from './HqFxRate';
import HqApprovalPolicy from './HqApprovalPolicy';
import HqApprovalRequest from './HqApprovalRequest';
...
// 발송된 급여 명세서 (엑셀 업로드 → 메일, Hyvision One 사용자 매칭 시)
...
(Tenant as any).hasMany(MfgBranch, { foreignKey: 'tenant_id', as: 'mfgBranches' });
(MfgBranch as any).belongsTo(Tenant, { foreignKey: 'tenant_id', as: 'tenant' });
(Company as any).hasMany(MfgBranch, { foreignKey: 'company_id', as: 'mfgBranches' });
(MfgBranch as any).belongsTo(Company, { foreignKey: 'company_id', as: 'company' });
(MfgBranch as any).hasMany(InventoryLocation, { foreignKey: 'branch_id', as: 'warehouses' });
(InventoryLocation as any).belongsTo(MfgBranch, { foreignKey: 'branch_id', as: 'branch' });

(Tenant as any).hasMany(MfgBinLocation, { foreignKey: 'tenant_id', as: 'mfgBinLocations' });
(MfgBinLocation as any).belongsTo(Tenant, { foreignKey: 'tenant_id', as: 'tenant' });
(Company as any).hasMany(MfgBinLocation, { foreignKey: 'company_id', as: 'mfgBinLocations' });
(MfgBinLocation as any).belongsTo(Company, { foreignKey: 'company_id', as: 'company' });
(InventoryLocation as any).hasMany(MfgBinLocation, { foreignKey: 'warehouse_id', as: 'bins' });
(MfgBinLocation as any).belongsTo(InventoryLocation, { foreignKey: 'warehouse_id', as: 'warehouse' });

(Tenant as any).hasMany(MfgDocumentSequence, { foreignKey: 'tenant_id', as: 'mfgDocumentSequences' });
(MfgDocumentSequence as any).belongsTo(Tenant, { foreignKey: 'tenant_id', as: 'tenant' });
(Company as any).hasMany(MfgDocumentSequence, { foreignKey: 'company_id', as: 'mfgDocumentSequences' });
(MfgDocumentSequence as any).belongsTo(Company, { foreignKey: 'company_id', as: 'company' });
(MfgBranch as any).hasMany(MfgDocumentSequence, { foreignKey: 'branch_id', as: 'documentSequences' });
(MfgDocumentSequence as any).belongsTo(MfgBranch, { foreignKey: 'branch_id', as: 'branch' });
(AcFinancialYear as any).hasMany(MfgDocumentSequence, { foreignKey: 'financial_year_id', as: 'mfgDocumentSequences' });
(MfgDocumentSequence as any).belongsTo(AcFinancialYear, { foreignKey: 'financial_year_id', as: 'financialYear' });

(Tenant as any).hasMany(MfgStockLedgerEntry, { foreignKey: 'tenant_id', as: 'mfgStockLedgerEntries' });
(MfgStockLedgerEntry as any).belongsTo(Tenant, { foreignKey: 'tenant_id', as: 'tenant' });
(Company as any).hasMany(MfgStockLedgerEntry, { foreignKey: 'company_id', as: 'mfgStockLedgerEntries' });
(MfgStockLedgerEntry as any).belongsTo(Company, { foreignKey: 'company_id', as: 'company' });
(Product as any).hasMany(MfgStockLedgerEntry, { foreignKey: 'product_id', as: 'mfgStockLedgerEntries' });
(MfgStockLedgerEntry as any).belongsTo(Product, { foreignKey: 'product_id', as: 'product' });
(InventoryLocation as any).hasMany(MfgStockLedgerEntry, { foreignKey: 'warehouse_id', as: 'mfgStockLedgerEntries' });
(MfgStockLedgerEntry as any).belongsTo(InventoryLocation, { foreignKey: 'warehouse_id', as: 'warehouse' });
(MfgBinLocation as any).hasMany(MfgStockLedgerEntry, { foreignKey: 'bin_id', as: 'stockLedgerEntries' });
(MfgStockLedgerEntry as any).belongsTo(MfgBinLocation, { foreignKey: 'bin_id', as: 'bin' });

(MfgPurchaseRequisition as any).hasMany(MfgPurchaseRequisitionItem, { foreignKey: 'requisition_id', as: 'items' });
(MfgPurchaseRequisitionItem as any).belongsTo(MfgPurchaseRequisition, { foreignKey: 'requisition_id', as: 'requisition' });
(MfgBranch as any).hasMany(MfgPurchaseRequisition, { foreignKey: 'branch_id', as: 'purchaseRequisitions' });
(MfgPurchaseRequisition as any).belongsTo(MfgBranch, { foreignKey: 'branch_id', as: 'branch' });
(Department as any).hasMany(MfgPurchaseRequisition, { foreignKey: 'department_id', as: 'purchaseRequisitions' });
(MfgPurchaseRequisition as any).belongsTo(Department, { foreignKey: 'department_id', as: 'department' });

(MfgPurchaseOrder as any).hasMany(MfgPurchaseOrderItem, { foreignKey: 'po_id', as: 'items' });
(MfgPurchaseOrderItem as any).belongsTo(MfgPurchaseOrder, { foreignKey: 'po_id', as: 'purchaseOrder' });
(MfgPurchaseRequisition as any).hasMany(MfgPurchaseOrder, { foreignKey: 'requisition_id', as: 'purchaseOrders' });
(MfgPurchaseOrder as any).belongsTo(MfgPurchaseRequisition, { foreignKey: 'requisition_id', as: 'requisition' });
(Partner as any).hasMany(MfgPurchaseOrder, { foreignKey: 'partner_id', as: 'purchaseOrders' });
(MfgPurchaseOrder as any).belongsTo(Partner, { foreignKey: 'partner_id', as: 'supplier' });

(MfgGoodsReceipt as any).hasMany(MfgGoodsReceiptItem, { foreignKey: 'grn_id', as: 'items' });
(MfgGoodsReceiptItem as any).belongsTo(MfgGoodsReceipt, { foreignKey: 'grn_id', as: 'goodsReceipt' });
(MfgPurchaseOrder as any).hasMany(MfgGoodsReceipt, { foreignKey: 'po_id', as: 'goodsReceipts' });
(MfgGoodsReceipt as any).belongsTo(MfgPurchaseOrder, { foreignKey: 'po_id', as: 'purchaseOrder' });
(MfgPurchaseOrderItem as any).hasMany(MfgGoodsReceiptItem, { foreignKey: 'po_item_id', as: 'grnItems' });
(MfgGoodsReceiptItem as any).belongsTo(MfgPurchaseOrderItem, { foreignKey: 'po_item_id', as: 'poItem' });

(MfgGoodsReceipt as any).hasMany(MfgQualityInspection, { foreignKey: 'grn_id', as: 'inspections' });
(MfgQualityInspection as any).belongsTo(MfgGoodsReceipt, { foreignKey: 'grn_id', as: 'goodsReceipt' });
(MfgGoodsReceiptItem as any).hasMany(MfgQualityInspection, { foreignKey: 'grn_item_id', as: 'inspections' });
(MfgQualityInspection as any).belongsTo(MfgGoodsReceiptItem, { foreignKey: 'grn_item_id', as: 'grnItem' });

(Tenant as any).hasMany(MfgDocumentAuditLog, { foreignKey: 'tenant_id', as: 'mfgDocumentAuditLogs' });
(MfgDocumentAuditLog as any).belongsTo(Tenant, { foreignKey: 'tenant_id', as: 'tenant' });
(Company as any).hasMany(MfgDocumentAuditLog, { foreignKey: 'company_id', as: 'mfgDocumentAuditLogs' });
(MfgDocumentAuditLog as any).belongsTo(Company, { foreignKey: 'company_id', as: 'company' });
(User as any).hasMany(MfgDocumentAuditLog, { foreignKey: 'actor_user_id', as: 'mfgAuditActions' });
(MfgDocumentAuditLog as any).belongsTo(User, { foreignKey: 'actor_user_id', as: 'actor' });

// Manufacturing Phase 2–5
(MfgSalesOrder as any).hasMany(MfgSalesOrderItem, { foreignKey: 'so_id', as: 'items' });
(MfgSalesOrderItem as any).belongsTo(MfgSalesOrder, { foreignKey: 'so_id', as: 'salesOrder' });
(Partner as any).hasMany(MfgSalesOrder, { foreignKey: 'partner_id', as: 'salesOrders' });
(MfgSalesOrder as any).belongsTo(Partner, { foreignKey: 'partner_id', as: 'customer' });

(MfgSalesOrder as any).hasMany(MfgStockReservation, { foreignKey: 'so_id', as: 'reservations' });
(MfgStockReservation as any).belongsTo(MfgSalesOrder, { foreignKey: 'so_id', as: 'salesOrder' });
(MfgSalesOrderItem as any).hasMany(MfgStockReservation, { foreignKey: 'so_item_id', as: 'reservations' });
(MfgStockReservation as any).belongsTo(MfgSalesOrderItem, { foreignKey: 'so_item_id', as: 'soItem' });

(MfgSalesOrder as any).hasMany(MfgDeliveryChallan, { foreignKey: 'so_id', as: 'deliveryChallans' });
(MfgDeliveryChallan as any).belongsTo(MfgSalesOrder, { foreignKey: 'so_id', as: 'salesOrder' });
(MfgDeliveryChallan as any).hasMany(MfgDeliveryChallanItem, { foreignKey: 'challan_id', as: 'items' });
(MfgDeliveryChallanItem as any).belongsTo(MfgDeliveryChallan, { foreignKey: 'challan_id', as: 'challan' });

(MfgBom as any).hasMany(MfgBomVersion, { foreignKey: 'bom_id', as: 'versions' });
(MfgBomVersion as any).belongsTo(MfgBom, { foreignKey: 'bom_id', as: 'bom' });
(MfgBomVersion as any).hasMany(MfgBomItem, { foreignKey: 'bom_version_id', as: 'items' });
(MfgBomItem as any).belongsTo(MfgBomVersion, { foreignKey: 'bom_version_id', as: 'bomVersion' });

(MfgRouting as any).hasMany(MfgRoutingOperation, { foreignKey: 'routing_id', as: 'operations' });
(MfgRoutingOperation as any).belongsTo(MfgRouting, { foreignKey: 'routing_id', as: 'routing' });
(MfgRoutingOperation as any).belongsTo(MfgWorkCenter, { foreignKey: 'work_center_id', as: 'workCenter' });

(MfgProductionPlan as any).hasMany(MfgMaterialRequirement, { foreignKey: 'plan_id', as: 'requirements' });
(MfgMaterialRequirement as any).belongsTo(MfgProductionPlan, { foreignKey: 'plan_id', as: 'plan' });

(MfgWorkOrder as any).hasMany(MfgMaterialIssue, { foreignKey: 'work_order_id', as: 'materialIssues' });
(MfgMaterialIssue as any).belongsTo(MfgWorkOrder, { foreignKey: 'work_order_id', as: 'workOrder' });
(MfgMaterialIssue as any).hasMany(MfgMaterialIssueItem, { foreignKey: 'issue_id', as: 'items' });
(MfgMaterialIssueItem as any).belongsTo(MfgMaterialIssue, { foreignKey: 'issue_id', as: 'materialIssue' });

(MfgWorkOrder as any).hasMany(MfgProductionEntry, { foreignKey: 'work_order_id', as: 'productionEntries' });
(MfgProductionEntry as any).belongsTo(MfgWorkOrder, { foreignKey: 'work_order_id', as: 'workOrder' });

(MfgAutoJournalLink as any).belongsTo(GlVoucher, { foreignKey: 'gl_voucher_id', as: 'glVoucher' });
