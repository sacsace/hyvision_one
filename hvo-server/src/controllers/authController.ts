...
import { resolveIsNotifierAuth, isHvoNotifierClient } from '../constants/authClients';
import {
...
export const register = async (req: Request, res: Response) => {
  if (isSingleCompanyMode()) {
    return res.status(403).json({
      success: false,
      message: '단일 회사 모드에서는 신규 가입(테넌트 생성)이 비활성화되어 있습니다.',
    });
  }
  ...
  try {
    ...
    const domain = `${subdomain}.hvo.local`;
...
export const refreshToken = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const isNotifier = isHvoNotifierClient(authClient);
...
export const logout = async (req: AuthRequest, res: Response) => {
  try {
    const user = req.user;
    const isNotifier = isHvoNotifierClient((req as any).authClient || (req as any).user?.client);
