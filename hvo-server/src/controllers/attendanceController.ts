...
export const checkIn = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (isManualAttendanceDisabled(company)) {
      return res.status(403).json({
        success: false,
        message: 'HeresNow 연동 사용자는 Hyvision One에서 직접 출근할 수 없습니다. HeresNow 기록만 조회할 수 있습니다.'
      });
    }
...
export const checkOut = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (isManualAttendanceDisabled(company)) {
      return res.status(403).json({
        success: false,
        message: 'HeresNow 연동 사용자는 Hyvision One에서 직접 퇴근할 수 없습니다. HeresNow 기록만 조회할 수 있습니다.',
      });
    }
