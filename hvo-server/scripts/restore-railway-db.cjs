 ...
 *   node scripts/restore-railway-db.cjs ..\backup\hvo_db.dump
 *   node scripts/restore-railway-db.cjs ..\backup\hvo_db.dump --clean
 *
...
const defaultDump = path.join(__dirname, '..', '..', 'backup', 'hvo_db.dump');
...
function main() {
  ...
  if (!fs.existsSync(dumpPath)) {
    ...
    console.error('   예: backup/hvo_db.dump 또는 pg_dump -Fc 로 만든 파일');
    process.exit(1);
  }
