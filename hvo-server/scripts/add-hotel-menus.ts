...
async function addHotelMenus() {
  try {
    ...
    if (!createdHotel) {
      ...
      console.log('ℹ️  기존 기숙사 관리 메뉴가 있어 업데이트했습니다.');
    } else {
      console.log('✅ 기숙사 관리 메뉴 생성 완료');
    }
    ...
    if (roomReservation) {
      ...
      console.log('✅ 객실 예약 관리 메뉴를 기숙사 관리 하위로 이동했습니다.');
    } else {
      console.log('⚠️  객실 예약 관리 메뉴를 찾지 못했습니다. 수동 확인이 필요합니다.');
    }

    console.log('🎉 기숙사 관리 메뉴 추가 완료');
    process.exit(0);
  } catch (error) {
    console.error('❌ 기숙사 관리 메뉴 추가 실패:', error);
    process.exit(1);
  }
}
