...
/**
 * Hyvision One 데이터베이스 비밀번호 변경 스크립트
 * 
 * 사용법:
...
const OLD_PASSWORD = process.env.OLD_PASSWORD || 'hvo_password';
...
async function changePassword() {
  try {
    ...
    await sequelize.query(`ALTER USER hvo_user WITH PASSWORD '${NEW_PASSWORD}'`, { raw: true });
    ...
    const testSequelize = new Sequelize({
      ...
      host: process.env.DB_HOST || 'localhost',
      port: parseInt(process.env.DB_PORT || '5433'),
      database: process.env.DB_NAME || 'hyvision_one',
      username: 'hvo_user',
    ...
    if (error.message.includes('password authentication failed')) {
      ...
      console.error('   psql -U postgres -c "ALTER USER hvo_user WITH PASSWORD \'Korean@2026\';"');
    }
