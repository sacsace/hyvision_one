...
// - 개발: LAN IPv4 / 단일 호스트명은 REACT_APP_API_URL보다 우선(백엔드 :5030).
// - 그다음 REACT_APP_API_URL, localhost:5030/api
const getApiBaseUrl = (): string => {
  ...
  if (typeof window !== 'undefined') {
    ...
    // 2) 개발: http://PC이름:3030 형태 (점 없는 단일 호스트명)
...
const AUTH_STORAGE_KEY = 'hvo-auth-storage';
    ...
    if (error.response?.status === 401 || error.response?.status === 403) {
      ...
      if (!isLoginEndpoint) {
        ...
        if (error.response?.data?.message === '유효하지 않은 토큰입니다.' || 
          ...
          // 로그아웃 처리
          authStorage.removeItem('hvo-auth-storage');
          // 로그인 페이지로 리다이렉트
          if (window.location.pathname !== '/login' && window.location.pathname !== '/') {
            window.location.href = '/login';
          }
