...
export function useBrowserDesktopNotifications(userId?: number) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('hvo-notification-prefs-updated', onPrefsUpdated);

    return () => {
      cancelled = true;
      window.removeEventListener('hvo-notification-prefs-updated', onPrefsUpdated);
    };
  }, [userId]);
...
export async function enableBrowserNotificationsFromSettings(): Promise<boolean> {
  ...
  window.dispatchEvent(new Event('hvo-notification-prefs-updated'));
  return ok;
}
