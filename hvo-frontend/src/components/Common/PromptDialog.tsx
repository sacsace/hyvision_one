...
/**
 * Hyvision One **기본 입력 다이얼로그** (`window.prompt` 대체).
 * 레이아웃은 `ConfirmDialog` 와 동일 셸(`hvoDialogShell`)을 공유합니다.
 */
const PromptDialog: React.FC<PromptDialogProps> = ({
  ...
  return (
    <Dialog
      open={open}
      onClose={onCancel}
      maxWidth={HVO_PROMPT_DIALOG_MAX_WIDTH}
      fullWidth
      scroll="paper"
      slotProps={{
        backdrop: {
    ...
    >
      <DialogTitle sx={getHvoDialogTitleRowSx(theme)}>
        <Box sx={getHvoDialogIconBoxSx(theme, theme.palette.primary.main, { tone: 'brand' })}>
      ...
      <DialogContent sx={getHvoDialogPromptContentSx(theme)}>
        {messageText ? (
      ...
      <DialogActions sx={getHvoDialogActionsSx(theme)}>
        ...
        <Button
          ...
          color="primary"
          disabled={!canSubmit}
          sx={getHvoDialogPrimaryConfirmButtonSx()}
        >
          {confirmLabel}
        </Button>
      </DialogActions>
