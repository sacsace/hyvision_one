...
import { hvoPageDescriptionSx, hvoPageTitleSx } from '../../theme/hvoLayout';

export type HvoPageHeaderProps = {
...
const HvoPageHeader: React.FC<HvoPageHeaderProps> = ({
  title,
  description,
  icon,
  iconPath,
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1.25, flex: { xs: '1 1 100%', sm: '1 1 280px' }, minWidth: 0 }}>
        ...
        <Box sx={{ minWidth: 0, flex: 1 }}>
          ...
          {description ? <Typography sx={hvoPageDescriptionSx}>{description}</Typography> : null}
        </Box>
      </Box>
      {actions ? (
        <Box
