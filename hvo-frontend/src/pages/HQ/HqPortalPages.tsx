...
export const HqPrivacyPage: React.FC = () => {
  const { t } = useTranslation();
  const [form, setForm] = useState({ purpose: '', retention_days: '365', legal_basis: '', contact_email: '' });
  const [msg, setMsg] = useState('');

  useEffect(() => {
    void hqPortalService
      .getPrivacy()
      .then((r) => {
        const d = r?.data || {};
        setForm({
          purpose: d.purpose || '',
          retention_days: String(d.retention_days ?? 365),
  ...
  return (
    <Box sx={hvoPageRootSx}>
      <HvoPageHeader title={t('hqPortal.privacyTitle')} description={t('hqPortal.privacyDesc')} />
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, maxWidth: 560 }}>
        <TextField
          size="small"
          label={t('hqPortal.purpose')}
          value={form.purpose}
        ...
        <Button
          ...
          sx={hvoBodyPrimaryBtnSx}
          onClick={async () => {
            await hqPortalService.updatePrivacy({
              ...form,
              retention_days: Number(form.retention_days),
            });
            setMsg(t('hqPortal.saved'));
          }}
        >
          {t('hqPortal.save')}
        </Button>
        {msg ? <Typography sx={{ fontSize: '0.8125rem' }}>{msg}</Typography> : null}
      </Box>
    </Box>
  );
};
