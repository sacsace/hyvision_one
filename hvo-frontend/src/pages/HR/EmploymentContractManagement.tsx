...
const CONTRACT_FILTER_OUTLINED = hvoOutlinedLabelProps;
const contractFilterFieldSx = { ...hvoSearchFieldSx, ...hvoFilterFieldHeightSx } as const;
...
const EmploymentContractManagement: React.FC = () => {
  ...
  return (
    <Box sx={{ ...hvoPageRootSx }}>
      <HvoPageHeader title={t('employmentContractManagement.pageTitle')} description={t('employmentContractManagement.description')} />
      ...
      <Card elevation={0} sx={hvoBodyCardSx}>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 1, px: { xs: 2, sm: 2.5 }, py: 1, bgcolor: '#FFFFFF' }}>
          ...
          {tab === 'contracts' && canManage ? (
            <Button variant="contained" disableElevation size="small" startIcon={<AddIcon fontSize="small" />} onClick={() => openWizard()} sx={hvoBodyPrimaryBtnSx}>
              ...
              <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                {(['all', 'in_progress', 'completed'] as MyContractFilterMode[]).map((mode) => (
                  <Button
                    ...
                    sx={myContractFilter === mode ? hvoBodyPrimaryBtnSx : hvoBodyOutlinedBtnSx}
                  >
                    {mode === 'all'
                      ...
                      <TableRow key={row.id} onClick={() => void openDetail(Number(row.id))} sx={{ cursor: 'pointer' }}>
                        ...
                        <TableCell align="center" className="action-cell" onClick={(e) => e.stopPropagation()}>
                          <Stack direction="row" spacing={0.5} justifyContent="center" flexWrap="wrap" useFlexGap>
                            {canSign ? (
                              <Button size="small" variant="outlined" startIcon={<DrawIcon fontSize="small" />} onClick={() => openSignDialog(Number(row.id))} sx={hvoBodyOutlinedBtnSx}>
                                {t('employmentContractManagement.actions.sign')}
                              </Button>
                            ) : (
                              <Button size="small" variant="outlined" startIcon={<VisibilityIcon fontSize="small" />} onClick={() => void openDetail(Number(row.id))} sx={hvoBodyOutlinedBtnSx}>
      ...
      <Dialog open={detailOpen} onClose={() => setDetailOpen(false)} fullWidth maxWidth="md">
        ...
        <DialogActions sx={{ px: 2, py: 1.5, gap: 1, flexWrap: 'wrap' }}>
          <Button onClick={() => setDetailOpen(false)} sx={hvoBodyOutlinedBtnSx}>
