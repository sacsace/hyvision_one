...
const DEPT_FORM_FIELD_SX = { ...hvoSearchFieldSx, ...hvoFilterFieldHeightSx } as const;
...
export const DepartmentManagementPanel: React.FC<{
  ...
  if (scopedCompanyId == null) {
    ...
    return <Box sx={hvoPageRootSx}>{selectCompanyBlock}</Box>;
  }
    ...
    <>
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 2.5, gap: 1 }}>
          <Button onClick={() => setDialogOpen(false)} sx={hvoBodyOutlinedBtnSx}>
  ...
  return (
    <Box sx={hvoPageRootSx}>
      <HvoPageHeader title={t('departmentManagement.title')} description={t('departmentManagement.description')} />
      {content}
    </Box>
  );
};
