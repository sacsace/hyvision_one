...
const PAYROLL_FILTER_OUTLINED = hvoOutlinedLabelProps;
const payrollFilterFieldSx = { ...hvoSearchFieldSx, ...hvoFilterFieldHeightSx } as const;
...
const PayrollManagement: React.FC<PayrollManagementProps> = ({ payslipSendOnly = false }) => {
  ...
  const [payslipHeaderLayout, setPayslipHeaderLayout] = useState<PayslipHeaderLayout>(() => {
    const saved = localStorage.getItem('hvo.payslipHeaderLayout');
  ...
  return (
    <Box sx={{ ...hvoPageRootFullBleedSx, flex: 1, minWidth: 0 }}>
      <HvoPageHeader
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 1, minWidth: 0 }}>
            ...
            <TextField
              ...
              onChange={(event) => {
                ...
                localStorage.setItem('hvo.payslipHeaderLayout', layout);
              }}
              sx={{ minWidth: 150 }}
      ...
      <Box sx={hvoBodyListZoneSx}>
        {loading ? (
          ...
          <Box sx={{ ...hvoBodyListTableSx, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <Box
              sx={{
                px: { xs: 2, sm: 2.5 },
                py: 1.5,
