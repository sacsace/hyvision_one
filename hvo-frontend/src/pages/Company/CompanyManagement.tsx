...
import {
  ...
  hvoTableBodyRowSx,
  hvoTableScrollSx,
  hvoBodyPaginationSx } from '../../theme/hvoLayout';
import {
...
type CompanyDialogSectionKey = 'tax' | 'bank' | 'transfer' | 'images';
...
const COMPANY_FILTER_OUTLINED = hvoOutlinedLabelProps;
const companyFilterFieldSx = { ...hvoSearchFieldSx, ...hvoFilterFieldHeightSx } as const;

const companyTableBodyRowSx: SxProps<Theme> = (theme) => {
  const base = typeof hvoTableBodyRowSx === 'function' ? hvoTableBodyRowSx(theme) : hvoTableBodyRowSx;
...
type CompanySortKey = 'name' | 'ceo_name' | 'industry' | 'employee_count' | 'hvo_start';
...
function descendingComparator(a: Company, b: Company, orderBy: CompanySortKey): number {
  ...
  if (orderBy === 'hvo_start') {
    const as = a.hvo_start_date || '';
    const bs = b.hvo_start_date || '';
    if (bs < as) return -1;
    if (bs > as) return 1;
    return 0;
  }
...
const CompanyManagement: React.FC = () => {
  ...
  const { user } = useStore();
  const showMultiCompanyAdmin = user?.role === 'root' && !isSingleCompanyMode();
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      ...
      try {
        ...
        // 단일회사 모드 또는 일반 사용자는 본인 회사만
        if (showMultiCompanyAdmin) {
    ...
    fetchCompanies();
  }, [user?.role, user?.company_id, user?.id, menuFlags.menusLoading, menuFlags.canRead, showMultiCompanyAdmin]);

  // 회사 추가
  const handleAdd = () => {
    if (!showMultiCompanyAdmin) return;
  ...
  const handleSave = async () => {
    ...
    try {
      if (dialogMode === 'add') {
                // 회사 등록 시 Hyvision One 시스템 사용 가능하도록 설정
        const companyData = {
          ...formData,
        ...
        if (isSingleCompanyMode()) {
          companyData.hvo_start_date = '';
          companyData.hvo_end_date = '';
          companyData.login_period_start = '';
          companyData.login_period_end = '';
        }
                
        const response = await api.post('/company', companyData);
                if (response.data.success) {
          const baseMsg = '회사가 성공적으로 등록되었습니다. Hyvision One 시스템을 사용할 수 있습니다.';
          setSuccess(
        ...
        // 단일 회사 모드: 사용 기간을 두지 않음 (기존 값도 저장 시 비움)
        if (isSingleCompanyMode()) {
          updateData.hvo_start_date = '';
          updateData.hvo_end_date = '';
          updateData.login_period_start = '';
          updateData.login_period_end = '';
        }
  ...
  // 필터링된 회사 목록 - Hyvision One 시스템을 사용할 수 있는 회사들 (활성 상태)
  ...
  const tdSx = (key: string) => ({
    ...companyColBaseSx(key),
    textOverflow:
      key === 'name' || key === 'ceo_name' || key === 'industry' || key === 'hvo_start'
        ? ('ellipsis' as const)
        : undefined });
        ...
        <>
          ...
          >
            {[filteredCompanies.length, companies.length].map((value, idx) => (
              <Card key={idx === 0 ? 'hvo' : 'total'} elevation={0} sx={hvoKpiCardSx}>
                <CardContent sx={{ py: 2.25, px: 2.5, '&:last-child': { pb: 2.25 } }}>
                  <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 600, letterSpacing: '0.02em' }}>
                    {idx === 0 ? t('companyManagement.hvoUsageCompanies') : t('companyManagement.totalCompanies')}
                  </Typography>
          ...
          <Card elevation={0} sx={hvoBodyCardSx}>
            ...
            <Box
              sx={{
                ...
                py: 2,
                bgcolor: '#FFFFFF',
                ...(hvoSearchFieldSx as Record<string, unknown>),
                display: 'grid',
                gridTemplateColumns: {
                  xs: '1fr',
          ...
          <Box sx={hvoBodyListZoneSx}>
            {loading ? (
              ...
              <TableContainer sx={{ ...hvoBodyListTableSx, ...hvoTableScrollSx }}>
                ...
                >
                  <TableHead
                    sx={(theme) => {
                      const headBase =
                        typeof hvoTableHeadHighlightSx === 'function'
                          ? hvoTableHeadHighlightSx(theme)
                          : hvoTableHeadHighlightSx;
                      return {
                        ...(headBase as object),
                        '& .MuiTableCell-head': {
                  ...
                  >
                    <TableRow>
                      ...
                      {renderHeadSortCell('hvo_start', t('companyManagement.hvoUsagePeriod'))}
                  ...
                  <TableBody sx={companyTableBodyRowSx}>
                    {displayedCompanies.map((company) => (
                      ...
                      >
                        ...
                        <TableCell align={compColTableAlign('hvo_start')} sx={tdSx('hvo_start')}>
                          <Typography
                            component="span"
                            noWrap
                            title={
                              company.hvo_start_date && company.hvo_end_date
                                ? `${company.hvo_start_date} ~ ${company.hvo_end_date}`
                                : '-'
                            }
              ...
              {listViewMode === 'page' ? (
                ...
                <Box sx={{ ...hvoBodyPaginationSx, py: 1 }}>
          ...
          <Card elevation={0} sx={hvoBodyCardSx}>
            <CardContent sx={{ p: { xs: 3, sm: 4 } }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                {/* 로그인 시간대 */}
                {(companies[0].login_time_start || companies[0].login_time_end || companies[0].timezone) && (
                  <Box>
                    <Divider sx={{ my: 3, borderWidth: 1.5, borderColor: 'divider' }} />
                    <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 2, fontSize: '0.75rem' }}>로그인 시간</Typography>
                    <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 4 }}>
