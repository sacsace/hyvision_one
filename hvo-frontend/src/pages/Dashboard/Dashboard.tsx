...
import {
  ...
  hvoDashboardWidgetCardSx,
  hvoDashboardWidgetGroupSx,
  hvoKpiCardSx,
  hvoPageRootSx } from '../../theme/hvoLayout';
...
const DASHBOARD_WIDGET_CARD_BASE_SX: SxProps<Theme> = {
  ...(hvoDashboardWidgetCardSx as object),
  height: '100%',
  display: 'flex',
...
const Dashboard: React.FC = () => {
    ...
    try {
      ...
      if (serverMessage?.includes('이미 출근')) {
        ...
        setAttendanceMessage(
          ...
          '백엔드(hvo-server)가 주소 ' + (api.defaults.baseURL || '미설정').replace(/\/api\/?$/, '') + ' 에서 실행 중인지 확인하세요.'
        );
        setAttendanceSeverity('error');
        messageSet = true;
      } else {
  ...
  const StatCard = ({ title, value, icon, trend, color = 'primary', onClick, showIfEmpty = false }: any) => {
    ...
    return (
      <Card
        elevation={0}
        sx={(th) => ({
          ...(hvoKpiCardSx as (theme: Theme) => Record<string, unknown>)(th),
          height: '100%',
          position: 'relative',
          overflow: 'hidden',
  ...
  return (
    <Box sx={hvoPageRootSx}>
      ...
      <Card elevation={0} sx={{ ...hvoBodyCardSx, mb: 2.5, overflow: 'hidden' }}>
        <Tabs
      ...
      <Card elevation={0} sx={{ ...hvoBodyCardSx, mb: 2.5 }}>
