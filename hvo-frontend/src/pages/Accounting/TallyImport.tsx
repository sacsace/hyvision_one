...
type ReconciliationData = {
  ...
  note: string;
  checks: {
    voucherCount: { status: string; source: number; hvo: number };
    debitTotal: { status: string; source: string; hvo: string; difference: string };
    creditTotal: { status: string; source: string; hvo: string; difference: string };
...
const TallyImport: React.FC = () => {
  ...
  return (
    <Box sx={hvoPageRootSx}>
      <HvoPageHeader title={t('tallyImport.title')} description={t('tallyImport.description')} />
      ...
      <Paper elevation={0} sx={{ ...hvoBodyCardSx, p: { xs: 1.25, sm: 2 }, mb: 1.5 }}>
      ...
      <Paper elevation={0} sx={{ ...hvoBodyCardSx, p: { xs: 1.5, sm: 2 }, borderRadius: 0 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: { xs: 'stretch', sm: 'center' }, flexDirection: { xs: 'column', sm: 'row' }, gap: 1 }}>
          ...
          <Button variant="outlined" startIcon={<ResetIcon fontSize="small" />} onClick={handleReset} sx={{ ...hvoBodyOutlinedBtnSx }}>
            {t('common.reset')}
          </Button>
          <Button
            variant="contained"
            disableElevation
            sx={{ ...hvoBodyPrimaryBtnSx, ml: { xs: 0, sm: 'auto' } }}
      ...
      {preview ? (
        <Paper elevation={0} sx={{ ...hvoBodyCardSx, mt: 1.5, p: { xs: 1.5, sm: 2 }, borderRadius: 0 }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
            <Button variant="contained" disableElevation sx={hvoBodyPrimaryBtnSx} onClick={() => void handleImport(true)} disabled={loading}>
        ...
        <Paper elevation={0} sx={{ ...hvoBodyCardSx, mt: 1.5, p: { xs: 1.5, sm: 2 }, borderRadius: 0 }}>
                ...
                <Stack direction="row" flexWrap="wrap" useFlexGap spacing={1} sx={{ alignItems: 'center' }}>
                  {(['all', 'error', 'warn', 'info'] as const).map((key) => (
                    <Button
                      ...
                      sx={{ ...hvoBodyOutlinedBtnSx, minWidth: 0, px: 1.25, ...(issueLevelFilter === key ? { boxShadow: 'none', color: '#fff' } : {}) }}
                    >
                      {t(`tallyImport.logFilter.${key}`)} ({issueCounts[key]})
                    </Button>
                  ))}
                </Stack>
                <Button size="small" variant="outlined" startIcon={<DownloadIcon fontSize="small" />} disabled={reportIssues.length === 0} onClick={() => void handleDownloadImportLog()} sx={hvoBodyOutlinedBtnSx}>
          ...
          {result?.dryRun ? (
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', mt: 1.5, gap: 1 }}>
              <Button variant="contained" disableElevation sx={hvoBodyPrimaryBtnSx} onClick={() => void handleImport(false)} disabled={loading || errorCount > 0}>
      ...
      {result && !result.dryRun ? (
        <Paper elevation={0} sx={{ ...hvoBodyCardSx, mt: 1.5, p: { xs: 1.5, sm: 2 }, borderRadius: 0 }}>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
            <Button variant="outlined" startIcon={<BooksIcon fontSize="small" />} sx={hvoBodyOutlinedBtnSx} onClick={() => navigate(booksHref)}>
              {t('tallyImport.booksLink')}
            </Button>
            <Button variant="outlined" sx={hvoBodyOutlinedBtnSx} disabled={!result.batchId || loading} onClick={() => void handleReconciliation()}>
              ...
              <Table size="small" sx={wizardTableSx}>
                <TableHead sx={hvoTableHeadHighlightSx}>
                  <TableRow>
                    ...
                    <TableCell align="right">{t('tallyImport.reconciliationColumns.hvo')}</TableCell>
                ...
                <TableBody>
                  <TableRow>
                    ...
                    <TableCell align="right">{reconciliation.checks.voucherCount.hvo}</TableCell>
                  ...
                  <TableRow>
                    ...
                    <TableCell align="right">{reconciliation.checks.debitTotal.hvo}</TableCell>
                  ...
                  <TableRow>
                    ...
                    <TableCell align="right">{reconciliation.checks.creditTotal.hvo}</TableCell>
