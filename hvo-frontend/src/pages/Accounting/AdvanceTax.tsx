...
const storageKey = (companyId: number | undefined, fy: number) =>
  `hvo.advanceTax.paid.${companyId || 0}.${fy}`;
...
const AdvanceTax: React.FC = () => {
  ...
  return (
    <Box sx={hvoPageRootSx}>
      <HvoPageHeader title={t('advanceTax.title')} description={t('advanceTax.description')} />
        ...
        <Box sx={hvoBodyFilterWrapSx}>
          ...
          >
            ...
            <Button
              ...
              onClick={() => void load()}
              disabled={loading}
              sx={{ ...hvoBodyPrimaryBtnSx, height: 40 }}
            >
      ...
      >
        ...
        <DialogContent dividers>
          {detailLoading ? (
            ...
            <Box sx={{ display: 'grid', gap: 2 }}>
              ...
              <TableContainer sx={{ ...hvoBodyListTableSx, ...hvoTableScrollSx, borderRadius: 0, boxShadow: 'none' }}>
                <Table size="small" sx={{ ...compactDetailTableSx, minWidth: 760 }}>
                  <TableHead sx={[hvoTableHeadHighlightSx, compactDetailHeadSx] as any}>
                    <TableRow>
        ...
        <DialogActions>
          <Button onClick={() => setDetail(null)} disabled={detailLoading} sx={hvoBodyOutlinedBtnSx}>
            {t('common.close')}
          </Button>
        </DialogActions>
      </Dialog>
