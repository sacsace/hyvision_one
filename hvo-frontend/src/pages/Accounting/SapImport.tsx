...
const SapImport: React.FC = () => {
  ...
  const saveGlMapping = async () => {
    if (!sourceGlCode.trim() || !selectedTargetAccount) {
      setError('SAP GL Code와 연결할 Hyvision One 원장을 선택하세요.');
      return;
    }
    setLoading(true);
    setError('');
  ...
  return (
    <Box sx={hvoPageRootSx}>
      <HvoPageHeader
        title="SAP Data 불러오기"
        description="SAP Excel/CSV를 검토한 뒤 Hyvision One 임시 전표로 변환합니다."
      />
      ...
      {inspection ? (
        <Paper elevation={0} sx={{ ...hvoBodyCardSx, mt: 1.5, p: { xs: 1.5, sm: 2 }, borderRadius: 0 }}>
          ...
          <Table size="small" sx={{ border: '1px solid #B4B4B4', '& .MuiTableCell-root': { borderColor: '#B4B4B4' } }}>
            <TableHead sx={hvoTableHeadHighlightSx}>
              <TableRow>
                <TableCell width="42%">Hyvision One 표준 필드</TableCell>
                <TableCell>원본 SAP 컬럼</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
        ...
        <Paper elevation={0} sx={{ ...hvoBodyCardSx, mt: 1.5, p: { xs: 1.5, sm: 2 }, borderRadius: 0 }}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '220px minmax(280px, 1fr) auto' }, gap: 1, alignItems: 'center' }}>
            ...
            <Autocomplete
              ...
              renderInput={(params) => <TextField {...params} label="Hyvision One 원장 계정" />}
            />
