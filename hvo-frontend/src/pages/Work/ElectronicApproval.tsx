...
const APPROVAL_OUTLINED = hvoOutlinedLabelProps;
...
const ElectronicApproval: React.FC = () => {
  ...
  const renderAttachmentPreviewDialog = () => (
    ...
    >
      ...
      <DialogActions sx={{ px: 2, py: 1.5 }}>
        <Button onClick={closeAttachmentPreview} variant="outlined" sx={hvoBodyOutlinedBtnSx}>
          {t('common.close')}
        </Button>
        {attachmentPreview ? (
          <Button
  ...
  const handleDeleteApprovalType = (row: { id: number; name: string; code: string; is_system: boolean }) => {
    if (row.is_system) {
      setError(t('approval.typesSystemNoDelete'));
      return;
    }
    showConfirm(
      t('approval.typesDeleteConfirm', { name: getTypeLabel(row.code) }),
      async () => {
        try {
  ...
  return (
    <Box sx={{
      ...
      <HvoPageHeader title={t('approval.pageTitle')} mb={2} />

      {/* */}
      <Card elevation={0} sx={{ ...hvoBodyCardSx, mb: isListView || activeTab === 3 ? 2.5 : 2 }}>
        <Tabs
          value={activeTab}
          onChange={(e, newValue) => {
      ...
      {(viewMode === 'create' || viewMode === 'edit') && activeTab === 2 && (
        <Card elevation={0} sx={{ ...hvoBodyCardSx, mb: 3, maxWidth: '100%' }}>
      ...
      {isListView && (
        ...
        >
          {(
            ...
            <Card key={stat.label} elevation={0} sx={hvoKpiCardSx}>
