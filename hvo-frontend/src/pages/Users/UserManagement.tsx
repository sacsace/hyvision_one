...
import { toPayslipCompanyInfo, shortCompanyName } from '../HR/PayslipContent';
...
const USER_FILTER_OUTLINED = hvoOutlinedLabelProps;
const userFilterFieldSx = { ...hvoSearchFieldSx, ...hvoFilterFieldHeightSx } as const;
...
/** 생성/수정 폼 아코디언 — Hyvision One Body 리스트 영역 내 섹션 카드 */
...
const UserManagement: React.FC = () => {
  ...
  const showCompanyPicker = user?.role === 'root' && !isSingleCompanyMode();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      // root 멀티회사: 회사 필수 / 단일회사: 로그인 회사 자동 지정
      if (user?.role === 'root' && !(formData as { company_id?: number }).company_id) {
        if (isSingleCompanyMode() && user.company_id) {
          (formData as { company_id?: number }).company_id = Number(user.company_id);
        } else {
  ...
  return (
    <Box sx={{ ...hvoPageRootSx, borderRadius: 0 }}>
      <HvoPageHeader
      ...
      {/* 탭 네비게이션 */}
      <Card elevation={0} sx={{ ...hvoBodyCardSx, mb: isListView ? 3 : 2 }}>
          <Tabs
            value={pageTab}
            sx={{
              minHeight: 40,
            ...
            {loading ? (
              ...
              <>
                ...
                {listViewMode === 'page' ? (
                  ...
                  <Box sx={{ ...hvoBodyPaginationSx, justifyContent: 'flex-end' }}>
                    <Typography variant="body2" color="text.secondary">
      ...
      {pageTab === 2 && (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {(showCompanyPicker || (!isSingleCompanyMode() && user?.role === 'audit')) && (
            <Card elevation={0} sx={hvoBodyCardSx}>
                ...
                <AccordionDetails>
                  ...
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                    ...
                    <Button
                      ...
                      sx={{ alignSelf: 'flex-start', ...hvoBodyOutlinedBtnSx }}
                    >
