...
import {
  ...
  hvoBodyListTableSx,
  hvoBodyPaginationSx,
  hvoBodySectionPanelSx,
  hvoBodySectionPanelTitleSx,
  hvoSearchFieldSx,
  hvoFilterFieldHeightSx,
  hvoTableScrollSx,
  hvoTableHeadHighlightSx,
  hvoTableBodyRowSx } from '../../theme/hvoLayout';
import {
  Add as AddIcon,
...
const quotationFilterFieldSx = {
  ...
  ...hvoFilterFieldHeightSx } as const;
...
/** 견적서 고객 Autocomplete — 이름·연락처·주소·GST 단어 단위 부분 검색 */
function filterQuotationCustomerOptions(
  options: PartnerCustomer[],
  inputValue: string
): PartnerCustomer[] {
  const q = inputValue.trim().toLowerCase();
  if (!q) return options.slice(0, 80);
  const matched = options.filter((c) => {
    const fields = [
      c.name,
      c.email,
      c.phone,
    ...
    const words = blob.split(/[\s,;._/@-]+/).filter(Boolean);
...
const QuotationManagement: React.FC = () => {
  ...
  const loadPartners = useCallback(async () => {
    try {
      const data = await useReferenceDataStore.getState().fetchPartners(true);
  ...
  return (
    <Box sx={{ ...hvoPageRootSx }}>
      ...
      <Box sx={{
        ...
        <Card elevation={0} sx={hvoKpiCardSx}>
      ...
      {isCreating || isEditing ? (
        <Card elevation={0} sx={{ ...hvoBodyCardSx, mb: 3 }}>
              ...
              >
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, justifyContent: 'flex-end' }}>
                  {showDetailApproverToolbar && (
                    <>
                      <Button
                        ...
                        sx={{ ...hvoBodyPrimaryBtnSx, ...QUOTATION_APPROVE_BUTTON_SX }}
                      >
                  ...
                  {!quotationFormReadOnly && !canPartialContentEdit(selectedQuotation) && (
                    <Button type="submit" form="quotation-detail-form" variant="contained" color="primary" disableElevation sx={hvoBodyPrimaryBtnSx}>
                      {t('quotationManagement.save')}
                    </Button>
                  )}
                </Box>
          ...
          <Card elevation={0} sx={{ ...hvoBodyCardSx, mb: 3 }}>
            ...
            >
                ...
                <Button
                  ...
                  sx={{ ...hvoBodyOutlinedBtnSx, height: 40, whiteSpace: 'nowrap', width: { xs: '100%', sm: 'auto' }, minWidth: { sm: 120 } }}
                >
                  {t('common.reset')}
                </Button>
              </Box>
          ...
          <Box sx={hvoBodyListZoneSx}>
            <TableContainer sx={{ ...hvoBodyListTableSx, ...hvoTableScrollSx }}>
              ...
              >
                ...
                <TableBody sx={hvoTableBodyRowSx}>
      ...
      >
        ...
        <DialogActions>
          <Button onClick={closeRejectDialog} disabled={rejectSubmitting} sx={hvoBodyOutlinedBtnSx}>
            {t('quotationManagement.cancel')}
          </Button>
          <Button
            variant="contained"
  ...
  return (
    <Box component="form" id={formId} onSubmit={handleSubmit} sx={{ mt: 2 }}>
      ...
      >
        ...
        <Box className="quotation-pdf-section quotation-pdf-section-customer" sx={hvoBodySectionPanelSx}>
          <Box sx={hvoBodySectionPanelTitleSx} className="quotation-pdf-section-title">
              ...
              <Box>
                ...
                <Autocomplete
                  ...
                  filterOptions={(options, { inputValue }) =>
                    filterQuotationCustomerOptions(options, inputValue)
                  }
                  value={
        ...
        <Box className="quotation-pdf-section quotation-pdf-section-notes" sx={hvoBodySectionPanelSx}>
          <Box sx={hvoBodySectionPanelTitleSx} className="quotation-pdf-section-title">
        ...
        <Box
          ...
          sx={{ ...hvoBodySectionPanelSx, border: '1px solid #000' }}
        >
          <Box
            sx={{
              ...hvoBodySectionPanelTitleSx,
