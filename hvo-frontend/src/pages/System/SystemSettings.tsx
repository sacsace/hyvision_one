...
/** 설정 섹션 카드 — Hyvision One Body 외곽 톤 */
...
const SystemSettings: React.FC = () => {
  ...
  const [settings, setSettings] = useState({
    general: {
      companyName: 'Hyvision One',
      companyAbbreviation: 'HVO',
      companyLogo: '',
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const normalizedSettings = {
          ...
          general: {
            ...d.general,
            companyAbbreviation: String((d as any).general?.companyAbbreviation ?? 'HVO'),
            // 인도 서비스 기본 통화 고정
            currency: 'INR'
          },
          mailServer: {
  ...
  if (loading) {
    return (
      <Box sx={{ ...hvoPageRootSx, display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 320 }}>
                  ...
                  {backupFiles.map((file) => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        sx={{ ...hvoBodyOutlinedBtnSx, flexShrink: 0 }}
                      >
                        {downloadingBackupName === file.filename
