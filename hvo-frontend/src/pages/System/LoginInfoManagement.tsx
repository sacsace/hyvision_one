...
import {
  ...
  hvoBodyPrimaryBtnSx,
  hvoBodyListZoneSx,
  hvoBodyListTableSx } from '../../theme/hvoLayout';
...
const LOGIN_FILTER_OUTLINED = hvoOutlinedLabelProps;
const loginFilterFieldSx = { ...hvoSearchFieldSx, ...hvoFilterFieldHeightSx } as const;
...
const LoginInfoManagement: React.FC = () => {
  ...
  const loadCompanies = useCallback(async () => {
    try {
      ...
      const isRoot = user?.role === 'root' && !isSingleCompanyMode();
      const scopedList =
      ...
      <Card elevation={0} sx={hvoBodyCardSx}>
        ...
        >
          <Autocomplete
            ...
            disableClearable={isSingleCompanyMode() || user?.role !== 'root'}
        ...
        {!selectedCompanyId ? (
          ...
          <>
            ...
            {loading && loginInfos.length === 0 ? (
              ...
              <Box sx={{ ...hvoBodyListTableSx, p: { xs: 1, sm: 1.5 } }}>
                <LoginInfoExcelGrid
