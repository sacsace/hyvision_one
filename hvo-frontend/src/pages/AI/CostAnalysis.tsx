...
import { hvoPageRootSx } from '../../theme/hvoLayout';
...
import { hvoFilterToolbarSx, hvoSearchFieldSx, hvoInnerCardSx } from '../../theme/hvoLayout';
...
const CostAnalysis: React.FC = () => {
  ...
  return (
    <Box sx={{ ...hvoPageRootSx }}>
      ...
      {/* 필터 및 액션 */}
      <Card elevation={0} sx={{ mb: 3, ...hvoFilterToolbarSx, ...hvoSearchFieldSx, ...filterBarSx }}>
      ...
      <TabPanel value={activeTab} index={0}>
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 3 }}>
          {/* 총 비용 카드 */}
          <Card elevation={0} sx={{ ...hvoInnerCardSx, ...cardShellSx }}>
          ...
          {/* 월별 트렌드 */}
          <Card elevation={0} sx={{ gridColumn: 'span 2', ...hvoInnerCardSx, ...cardShellSx }}>
            <CardContent sx={{ py: 2.75, px: { xs: 2.25, sm: 3 }, '&:last-child': { pb: 2.75 } }}>
              <Typography component="h2" variant="h6" sx={sectionTitleSx}>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {analysisData?.aiInsights?.map((insight) => (
            <Card
              key={insight.id}
              elevation={0}
              sx={{ ...hvoInnerCardSx, ...cardShellSx, cursor: 'pointer' }}
      ...
      <TabPanel value={activeTab} index={3}>
        <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 3 }}>
          {analysisData?.benchmarks?.map((benchmark, index) => (
            <Card key={index} elevation={0} sx={{ ...hvoInnerCardSx, ...cardShellSx }}>
              <CardContent sx={{ py: 2.75, px: { xs: 2.25, sm: 3 }, '&:last-child': { pb: 2.75 } }}>
