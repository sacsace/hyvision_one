# Hyvision One Windows Tray Notifier (lightweight)
...
$AppName = "Hyvision One Notifier"
$DataDir = Join-Path $env:LOCALAPPDATA "HVO-Notifier"
...
function Show-LoginDialog([string]$DefaultApi) {
  ...
  $txtApi.Width = 370
  $txtApi.Text = if ($DefaultApi) { $DefaultApi } else { "http://localhost:5030/api" }
...
function Invoke-HvoLogin($ApiBase, $UserId, $Password) {
  $payload = [ordered]@{
    userid   = $UserId
    password = $Password
    client   = "hvo_notifier"
  ...
  $headers = @{
    "User-Agent"   = "HVO-Notifier/1.0"
    "X-HVO-Client" = "hvo_notifier"
  }
...
function Get-HvoNotifications($ApiBase, $Token) {
  $headers = @{
...
if ($Setup -or -not $config -or -not $config.token) {
  ...
  if (-not $login) { exit 0 }
  try {
    $config = Invoke-HvoLogin -ApiBase $login.apiBase -UserId $login.userid -Password $login.password
...
$notify.Text = $AppName
$iconPath = Join-Path $PSScriptRoot "hvo-notifier.ico"
...
$miOpen = $menu.Items.Add("Open Hyvision One")
...
function Sync-Notifications([bool]$ForceTip) {
  try {
    $rows = Get-HvoNotifications -ApiBase $config.apiBase -Token $config.token
  } catch {
...
$miOpen.add_Click({
  $url = if ($config.webBase) { $config.webBase } else { "http://localhost:3030" }
  Start-Process $url
})
