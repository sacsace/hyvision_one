...
**Nixpacks 설정 확인:**
```
║ start      │ echo 'This is a monorepo root. Please deploy hvo-server or msv- ║
...
**문제:**
- `hvo-frontend` 서비스가 **루트 디렉토리(`/`)**를 가리키고 있음
- 실제로는 **`hvo-frontend` 디렉토리**를 배포해야 함

**증거:**
- Nixpacks가 루트 `railway.toml`을 읽고 있음
- 루트 `railway.toml`의 `startCommand`가 실행됨
- `hvo-frontend/railway.toml`이나 `hvo-frontend/nixpacks.toml`이 사용되지 않음
...
**결과:**
- Railway가 `hvo-frontend` 디렉토리에서 배포
- `hvo-frontend/railway.toml` 또는 `hvo-frontend/nixpacks.toml` 사용
- 올바른 시작 명령 실행
...
- `hvo-backend` → `hvo-server`
- `hvo-frontend` → `hvo-frontend`
...
- `hvo-frontend` 서비스의 Root Directory가 설정되지 않음
- 루트 `railway.toml`의 `startCommand`가 실행되어 `exit 1`로 종료

**해결:**
- Railway 대시보드 → hvo-frontend → Settings
- Root Directory를 `hvo-frontend`로 설정
- 재배포

**결과:**
- `hvo-frontend` 디렉토리에서 정상적으로 배포
- 올바른 시작 명령 실행
- 서비스 정상 작동
